...
fun PermissionBanner(
    ...
    // On many OEM skins (Xiaomi/MIUI, Oppo, Vivo, etc.) the system kills the app's process
    // in the background unless it's exempted from battery optimization — without this,
    // scheduled alarms can silently stop firing even though they were registered correctly.
    val powerManager = context.getSystemService(Context.POWER_SERVICE) as? PowerManager
    val isIgnoringBatteryOptimizations = powerManager?.isIgnoringBatteryOptimizations(context.packageName) ?: true

    if (needsNotificationPermission || !canExactAlarm || !isIgnoringBatteryOptimizations) {
        Card(
            modifier = modifier
                .fillMaxWidth()
