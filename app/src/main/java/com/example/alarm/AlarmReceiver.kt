...
class AlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            ...
            // If a foreground service cannot be started, still show the notification as a fallback.
            android.util.Log.e("AlarmReceiver", "Failed to start ringing service", e)
            fallbackNotification(context, intent)
        }
    }

    private fun fallbackNotification(context: Context, intent: Intent) {
        ...
        val notificationId = (if (medicineId > 0) medicineId.toInt() * 100 + scheduledTime.hashCode() % 100 else 99999).let { if (it < 0) -it else it }
        showMedicineNotification(context, medicineId, medicineName, dosage, scheduledTime, instruction, dateString, isSnooze, notificationId)
    }
